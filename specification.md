...
### 2.1 構成ファイル一覧
- **`ginga.py`**: WEST EXPRESS 銀河（紀南コース・山陰コース）の空席状況を取得し、LINEへ通知（Broadcast）する統合スクリプト。引数でコース（kinan / sanin）の指定も可能。
- **`sunrise.py`**: サンライズ瀬戸・出雲の空席状況を取得し、LINEへ通知（Push）するスクリプト。当日の便に加えて `config.json` に指定された別日（有効な乗車日）も対象に取得します。
- **`config.json`**: 各路線の対象運行日を一元管理する設定ファイル。銀河の運行日およびサンライズの追加対象日を管理します。ダッシュボード上からも直接追加・編集が可能です。
...
- **`dashboard.js`**: `history.json` および各路線のログファイル（`log_kinan.json` 等）を非同期で読み込み、以下の情報を表示します。
  - 最新の空席状況一覧（列車名・対象日・区間ごとの各席種ステータス）
  - 席種別空席出現率（ドーナツグラフ - Chart.js）
  - 時間帯別空席検知件数（棒グラフ - Chart.js）
  - 直近の空席検知タイムライン（初期5件表示、クリックで最大20件まで展開）
  - アクセスログ詳細（タイムラインと同様、初期5件表示から最大20件まで展開可能）

## 4. 実行フローと自動化要件
各スクリプトは、外部のタスクスケジューラ（cron、Windows Task Scheduler、GitHub Actions等）を用いて定期実行されることを想定しています。

1. スケジューラが各Pythonスクリプト（`ginga.py`や`sunrise.py`等）を実行。
